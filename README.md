...
rankstr_mpi_comm_split is a function like MPI_Comm_split, but it takes a variable-length string as the color argument insead of a integer value.
...
The caller must provide two distinct tag values that are safe for the function can use for its point-to-point messages.
...
The next two parameters (1, 2) are tag values the function will use on the input communicator
for its point-to-point messages.  These two tags values must be distinct.
